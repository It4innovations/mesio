...
void computeNodeDomainDistribution(ElementStore *elements, NodeStore *nodes, DomainStore *domains, std::vector<int> neighborsWithMe)
{
	...
	for (int t = 1; t < info::env::threads; t++) {
		...
		if (ddistribution[t] < domainsDistrubtion.size()) {
			...
			ddatadistribution[t] = domainsDistrubtion[ddistribution[info::env::threads] - 1];
		}
	}
	++ddistribution[info::env::threads];
	ddatadistribution[info::env::threads] = domainsDistrubtion[ddistribution[info::env::threads] - 1];
