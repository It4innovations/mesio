...
//	for (auto it = info::config::input.contact_interfaces.begin(); it != info::config::input.contact_interfaces.end(); ++it) {
...
void computeWarpedNormals(SurfaceStore * surface)
{
	...
	std::vector<std::vector<Point> > normal(info::env::threads), base(info::env::threads), parameters(info::env::threads);
...
void exchangeContactHalo(SurfaceStore * surface, ContactStore *contact)
{
	...
	for (size_t n = 0, i = 0; n < contact->neighbors.size(); ++n, i = 0) {
		...
		contact->surfaces[n]->fID = new serializededata<esint, esint>(1, tarray<esint>(info::env::threads, rBuffer[n][i]));
		memcpy(contact->surfaces[n]->fID->datatarray().data(), rBuffer[n].data() + i + 1, sizeof(esint) * rBuffer[n][i]);
		contact->surfaces[n]->body = new serializededata<esint, esint>(1, tarray<esint>(info::env::threads, rBuffer[n][i]));
		memcpy(contact->surfaces[n]->body->datatarray().data(), rBuffer[n].data() + i + 1 + rBuffer[n][i], sizeof(esint) * rBuffer[n][i]);
		contact->surfaces[n]->epointers = new serializededata<esint, Element*>(1, tarray<Element*>(info::env::threads, rBuffer[n][i]));
		...
		// receive enodes
		std::vector<std::vector<esint> > edist(info::env::threads), enodes(info::env::threads);
		...
		size = rBuffer[n][i];
		contact->surfaces[n]->normal = new serializededata<esint, Point>(1, tarray<Point>(info::env::threads, size));
		...
		contact->surfaces[n]->parameters = new serializededata<esint, Point>(2, tarray<Point>(info::env::threads, 2 * size));
		...
		contact->surfaces[n]->base = new serializededata<esint, Point>(1, tarray<Point>(info::env::threads, size));
		...
		// receive global ids
		size = rBuffer[n][i];
		contact->surfaces[n]->nIDs = new serializededata<esint, esint>(1, tarray<esint>(info::env::threads, size));
		memcpy(contact->surfaces[n]->nIDs->datatarray().data(), rBuffer[n].data() + i + 1, sizeof(esint) * rBuffer[n][i]);
		i += 1 + rBuffer[n][i];

		// receive coordinates
		contact->surfaces[n]->coordinates = new serializededata<esint, Point>(1, tarray<Point>(info::env::threads, size));
