...
esint callParallelDecomposer(const ElementStore *elements, const NodeStore *nodes, std::vector<esint> &eframes, std::vector<esint> &eneighbors, std::vector<esint> &partition)
{
	...
	if (
			(info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::NONE) ||
			(info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::METIS && !METIS::islinked()) ||
			(info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::PARMETIS && !ParMETIS::islinked()) ||
			(info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::PTSCOTCH && !PTScotch::islinked())
			) {
	...
	if (info::mpi::size <= info::config::input.third_party_scalability_limit && info::config::input.decomposition.parallel_decomposer != DecompositionConfiguration::ParallelDecomposer::METIS) {
		std::vector<esint> edistribution = Communication::getDistribution<esint>(partition.size(), &MPITools::subset->across);

		switch (info::config::input.decomposition.parallel_decomposer) {
		...
		case DecompositionConfiguration::ParallelDecomposer::PARMETIS:
			...
			if (info::config::input.decomposition.parmetis_options.refinement) {
		...
		if (info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::METIS) {
		...
		if (info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::METIS) {
			if (info::mpi::rank == 0) {
				fixframes();
				info::config::input.decomposition.metis_options.continuous = 0;
				edgecut = METIS::call(info::config::input.decomposition.metis_options,
						gpartition.size(), gframes.data(), gneighbors.data(),
						0, NULL, NULL, info::mpi::size, gpartition.data());
				info::config::input.decomposition.metis_options.continuous = 1;
			}
		...
		if (
				info::config::input.decomposition.parallel_decomposer == DecompositionConfiguration::ParallelDecomposer::PARMETIS &&
				info::config::input.decomposition.parmetis_options.refinement) {
