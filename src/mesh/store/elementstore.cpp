...
void ElementStore::unpackFull(const char* &p)
{
	...
	if (notnull) {
		...
		epointers = new serializededata<esint, Element*>(1, tarray<Element*>(info::env::threads, distribution.process.size));
...
ElementData* ElementStore::appendData(int dimension, NamedData::DataType datatype, const std::string &name)
{
