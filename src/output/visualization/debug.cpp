...
{
	_path = utils::createDirectory({ info::config::output.path, "DEBUG_VISUALIZATION" });
...
{
	if (!info::config::output.debug || info::mesh->contact->sparseSide == NULL) {
		return;
	}
