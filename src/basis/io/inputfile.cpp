...
void InputFilePack::prepare()
{
	...
	{ // set a reasonable reorganization according to the stripe size setting (black magic)
		size_t stripe = info::config::input.stripe_size;
...
void InputFilePack::read()
{
	...
	for (size_t i = 0; i < files.size(); ++i) {
		...
		if (info::config::input.loader == InputConfiguration::LOADER::MPI) {
			MPILoader loader;
		...
		if (info::config::input.loader == InputConfiguration::LOADER::MPI_COLLECTIVE) {
		...
		if (info::config::input.loader == InputConfiguration::LOADER::POSIX) {
			POSIXLoader loader;
			if (MPITools::subset->within.rank == 0) {
				if (loader.open(MPITools::subset->across, paths[i])) {
					eslog::error("LOADER: cannot read file '%s'\n", paths[i].c_str());
