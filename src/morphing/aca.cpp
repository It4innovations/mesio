...
void LowRankBlock::generateBlocksImplicit(const Cluster *L, const Cluster *R, double eps, const RBFTargetConfiguration &configuration){
	...
	while(true){
		...
		double norm_new_row = mesio::MATH::vecDot(this->ncols, &cross_row->at(0));
		double norm_new_col = mesio::MATH::vecDot(this->nrows, &cross_col->at(0));
		double new_cross_norm = norm_new_row * norm_new_col;
		
		approx_norm += new_cross_norm;
		for ( esint i = 0; i < this->rank; ++i ) {
		  approx_norm += 2.0f * mesio::MATH::vecDot(this->ncols, &cross_row->at(0), &data_rows[i]->at(0)) * mesio::MATH::vecDot(this->nrows, &cross_col->at(0), &data_cols[i]->at(0));
		}
